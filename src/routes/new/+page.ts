...
export const load = (async ({ fetch, depends, url }) => {
    const jobReq = await fetch("https://hacker-news.firebaseio.com/v0/newstories.json")
    ...
    return {
        jobs: jobs,
        prevLink: page === 1 ? undefined : page - 1 === 1 ? "/new" : `/new?p=${page - 1}`,
        nextLink: hasNextLink ? `/new?p=${page + 1}` : undefined
    }
}) satisfies PageLoad
